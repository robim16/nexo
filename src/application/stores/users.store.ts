...
export const useUsersStore = defineStore('users', () => {
  ...
  async function checkIsFollowing(targetUserId: string) {
    ...
    try {
      ...
      const result = await repository.isFollowing(
        UserId.reconstitute(currentUserId),
        UserId.reconstitute(targetUserId)
      )
  ...
  async function unfollowUser(targetUserId: string) {
    ...
    try {
      ...
      if (profiles.value[targetUserId]) {
        profiles.value[targetUserId].followersCount = Math.max(
          0,
          profiles.value[targetUserId].followersCount - 1
        )
      }
      if (authStore.user) {
    ...
    try {
      ...
      if (profiles.value[currentUserId]) {
        ...
        if (data.displayName !== undefined)
          profiles.value[currentUserId].displayName = data.displayName
